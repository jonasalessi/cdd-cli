...
| TC-J3 | unit | `jvm.IsJDK("java")`, `IsJDK("javax")`, `IsJDK("")`, `IsJDK("javaxfoo.Bar")`. | `false` for all four — the prefixes end in a dot, so a bare or lookalike package does not match. |
