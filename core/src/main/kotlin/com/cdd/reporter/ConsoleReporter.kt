...
class ConsoleReporter : ReportGenerator {
    ...
    override fun generate(analysis: AggregatedAnalysis, config: CddConfig): String {
        return buildString {
            ...
            val overLimitPercent =
                if (analysis.totalClasses > 0) (overLimitCount.toDouble() / analysis.totalClasses) * 100 else 0.0
            appendLine(
                "Classes over limit:      ${if (overLimitCount > 0) RED else GREEN}$overLimitCount (${
                    String.format(
                        "%.1f",
                        overLimitPercent
                    )
            ...
            analysis.icpDistribution.forEach { (type, count) ->
                ...
                appendLine(
                    "${type.name.lowercase().replace('_', ' ').padEnd(20)}: ${
