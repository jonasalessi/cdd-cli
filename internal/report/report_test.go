...
// render is Write into a buffer, the way every test below uses it.
func render(t *testing.T, format string, res analyze.RunResult) string {
	t.Helper()
	var buf bytes.Buffer
	require.NoError(t, Write(&buf, format, res))
	return buf.String()
}

func TestWriteGolden(t *testing.T) {
	for _, format := range config.ReporterFormats() {
		for suffix, res := range goldenRuns() {
			t.Run(format+suffix, func(t *testing.T) {
				got := render(t, format, res)
...
func TestWriteUnknownFormat(t *testing.T) {
	err := Write(&bytes.Buffer{}, "yaml", fullRun())
	require.Error(t, err)
...
func TestWriteReportsWriteErrors(t *testing.T) {
	for _, format := range config.ReporterFormats() {
		assert.ErrorIs(t, Write(errWriter{}, format, fullRun()), errWrite, format)
	}
}
...
func TestJSONRoundTrip(t *testing.T) {
	...
	assert.Equal(t, Summary{Units: 3, Violations: 1}, doc.Summary)
...
func TestMarkdownShapesTheRun(t *testing.T) {
	out := render(t, config.FormatMarkdown, fullRun())
...
func TestXMLShapesTheRun(t *testing.T) {
	...
	assert.Contains(t, out, `<report root="/projects/shop" partial="false" elapsed_ms="1234">`)
